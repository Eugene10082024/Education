...
Кома́ндная оболо́чка (англ. shell) — командный интерпретатор, используемый в операционных системах семейства Unix, в котором пользователь может либо давать команды операционной системе по отдельности, либо запускать скрипты, состоящие из списка команд.

На текущий момент самая распространненная оболочка - bash

Как проверить какую оболочку:

        root@astra-01:~# root@astra-01:~# echo $SHELL
        /bin/bash
        root@astra-01:~#
Командная строка bash составляется из имени команды, за которым могут следовать ключи (опции) - указания модифицирующие поведение команды. Кроме ключей могут следовать аргументы (параментры) - названия объектов, над которыми должна быть выполнена команда (часто это файлы или папки)

В оболочке есть команда history которая выводит историю выпогнения команд. Вожно ее посмотреть и выполнить повторно нужные команды.
...
Выполнение команд из под другого пользователя  

sudo - команда которая позволяет вам запускать программы от имени других пользователей, а также от имени суперпользователя

          administrator@astra-01:~$ sudo adduser test
          Добавляется пользователь «test» ...
          Добавляется новая группа «test» (1003) ...
          Добавляется новый пользователь «test» (1002) в группу «test» ...
...
Повышение прав до root
          ...
          administrator@astra-01:~$ sudo -i
          root@astra-01:~# whoiam
...
Полный список ключей можно получить командой:
 ...
 Посмотр свойств файла
            
            file <name_file>
            
            root@astra-01:~# touch file01
            root@astra-01:~# file file01
            file01: empty
            root@astra-01:~# file .dbus/session-bus/d65844f303e249d987658dcf0b4d5499-0
            .dbus/session-bus/d65844f303e249d987658dcf0b4d5499-0: ASCII text
            root@astra-01:~#           
...
Смена пароля пользователя
            ...
            passwd <user> - если утилита запускается из под root
            passwd - смена пароля у текущего пользователя.
            
### Работа с пользователями и группами пользователей (работа с правами root).
 ...
 [Создание пользователя с помощью утилиты useradd](https://github.com/Aleksey-10081967/Education/tree/main/lesson-2/work-user.md)
