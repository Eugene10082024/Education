...
В 4-й версии IP-адрес представляет собой 32-битное число. IP-адрес состоит из 4 байт данных. Байт состоит из 8 бит (бит-это одна цифра, и это может быть только 1 или 0), поэтому у нас есть в общей сложности 32 бита для каждого IP-адреса. 
пример IP-адреса в двоичном формате: 10101100000100001111111000000001. 
Чтобы упростить представление представяют выше приведенный формат как 4 октета разделенных точкой:10101100.00010000.11111110.00000001
Чтобы еще больше упростить дело, для представления используется десятичное представление.
Оно используется для задания IP-адреса. Например : 172. 16. 254. 1 
В каждом октете - значение может быть в диапозоне от 0 до 255. 

Формула расчета (перевода из двоичной в десятичную систему исчисления):

                        <---
            2^7 2^6 2^5 2^4 2^3 2^2 2^1 2^0
             x   x  x    x   x   x   x   x 
             
            число октета = сумма (x*2^n-1)
            где x - значение бита (1 или 0)


Определение максимального десятичного чиcла в октете:
    
          11111111 -> 2^7+2^6+2^5+2^4+2^3+2^2+2^1+2^0=128+64+32+16+8+4+2+1=255

Перевод из дисятичного числа в двоичное можно посмотреть тут:

https://it-calc.ru/perevody/perevesti-99-iz-desyatichnoj-v-dvoichnuyu-sismemu-shisleniya/

Адреса IP, используемые в локальных (частных) сетях, относят к частным IP (серые IP адреса).

    10.0.0.0/8     - всего устройств - (2^24 - 2)  - 16 777 214
    172.16.0.0/12  - всего устройств -             -  1 048 576
    192.168.0.0/16 - всего устройств - (2^16 -2)   -     65 534

Адреса для внутреннего использования:

127.0.0.0/8 — используется для коммуникаций внутри хоста (см. localhost).

Подсеть-это процесс разделения более крупной сети на более мелкие подсети. Подсети разбивают большие сети на мелкие части, что является более эффективным и позволит сохранить большое количество адресов. 

Подсети получаются путем введения маски подсети.

Маска подсети в IPv4- это 32-разрядное число, которое сегментирует существующий IP-адрес в сети TCP/IP. 
Маска подсети делит IP-адрес на сетевой адрес и адрес хоста, таким образом, чтобы определить, какая часть IP-адреса зарезервирована для сети и какая часть доступна для использования хостом. 
Маска 24 - 11111111.11111111.11111111.00000000 или 255.255.255.0
После того, как задан IP-адрес и его маска подсети, можно определить сетевой адрес (подсеть) хоста.

      IP    - 192.168.  1.   3
      Маска - 255.255.255.   0 

Обычно калькуляторы подсетей легко доступны в интернете, которые помогают разделить IP-сеть на подсети. 

Таким образом IP-адрес состоит из двух частей: номера сети и номера узла в данной подсети (см рисунок ниже). 

![picture](dop_files/Pic4.png)
